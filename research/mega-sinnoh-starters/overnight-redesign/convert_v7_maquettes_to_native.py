...
SOURCE = WORK / 'v7-generated-underdrawings'
...
def make_frame1(frame0: Image.Image, species: str, view: str) -> Image.Image:
    """Two-frame Platinum-style breathing/bob motion with no new colors."""
    frame1 = Image.new('RGBA', frame0.size, (0, 0, 0, 0))
    # Opposed front/rear timing prevents all six sheets from feeling cloned.
    dx = 1 if species == 'torterra' and view == 'front' else 0
    dy = -1 if view == 'front' else 1
    if species == 'empoleon':
        dx = -1 if view == 'front' else 1
    frame1.alpha_composite(frame0, (dx, dy))
    return frame1
...
def preview(method: str, raw_root: Path) -> None:
    ...
    for species in SPECIES:
        for view in VIEWS:
            ...
            row += 1
    panel.save(WORK / f'target-native-v7-{method}-preview.png')


def build_method(method: str, resample: Image.Resampling) -> dict:
    raw_root = WORK / f'target-native-v7-{method}'
    sheet_root = WORK / f'target-native-v7-{method}-sheets'
    ...
    report_path = WORK / f'target-native-v7-{method}-conversion-report.json'
