...
def preview(method: str, raw_root: Path) -> None:
    ...
    for species in SPECIES:
        for view in VIEWS:
            ...
            row += 1
    panel.save(WORK / f'target-native-v8-{method}-preview.png')


def build_method(method: str, resample: Image.Resampling) -> dict:
    raw_root = WORK / f'target-native-v8-{method}'
    sheet_root = WORK / f'target-native-v8-{method}-sheets'
    ...
    report_path = WORK / f'target-native-v8-{method}-conversion-report.json'
