...
| `--stage-tod` | day | stage_ab, sprite_life (its first battle) and the other Plain-battle scenarios: time of day the launcher forces (`clock` = the map's own, which follows the host clock) |
| `--map` | the xMAP next to the ROM, or `<rom dir>/../build/main.nef.xMAP` | the build's `.xMAP`, for the RAM checks of all_backgrounds, mega and sprite_life, and for writing `debugFlags` (see "Lit, deformable sprites"); `none` = pixels only. A map from a different build is detected and ignored |
