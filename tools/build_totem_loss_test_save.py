...
def patch_partition(raw: bytearray, partition: int) -> str:
    ...
    for move_offset in MOVE_OFFSETS:
        p16(decrypted, move_offset, MOVE_MEMENTO)
    for pp_offset in PP_OFFSETS:
        decrypted[pp_offset] = 10
    for pp_up_offset in PP_UP_OFFSETS:
        decrypted[pp_up_offset] = 0

    decrypted[LEVEL_OFFSET] = 1
    for stat_offset in (
        CURRENT_HP_OFFSET,
        MAX_HP_OFFSET,
        ATTACK_OFFSET,
    ...
    if any(u16(roundtrip, move_offset) != MOVE_MEMENTO for move_offset in MOVE_OFFSETS):
        raise AssertionError(f"partition {partition}: move round-trip failed")
    if u16(roundtrip, CURRENT_HP_OFFSET) != 1 or u16(roundtrip, MAX_HP_OFFSET) != 1:
        raise AssertionError(f"partition {partition}: HP round-trip failed")
    ...
    return (
        ...
        f"level=1 hp=1/1 moves={MOVE_MEMENTO},{MOVE_MEMENTO},{MOVE_MEMENTO},{MOVE_MEMENTO}"
    )


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a checksum-valid one-Pokemon Platinum save for deterministic Totem loss testing."
    )
