...
ROM=${1:?usage: validate_totem_loss_persistence.sh ROM WEAK_RAW_SAVE SPECIES APPROACH_KEY OUT_DIR}
RAW=${2:?usage: validate_totem_loss_persistence.sh ROM WEAK_RAW_SAVE SPECIES APPROACH_KEY OUT_DIR}
SPECIES=${3:?usage: validate_totem_loss_persistence.sh ROM WEAK_RAW_SAVE SPECIES APPROACH_KEY OUT_DIR}
APPROACH=${4:?usage: validate_totem_loss_persistence.sh ROM WEAK_RAW_SAVE SPECIES APPROACH_KEY OUT_DIR}
OUT=${5:?usage: validate_totem_loss_persistence.sh ROM WEAK_RAW_SAVE SPECIES APPROACH_KEY OUT_DIR}
ROOT=$(cd "$(dirname "$0")/.." && pwd)
ROM=$(readlink -f "$ROM")
RAW=$(readlink -f "$RAW")
OUT=$(mkdir -p "$OUT" && readlink -f "$OUT")
SLUG=$(printf '%s' "$SPECIES" | tr '[:upper:]' '[:lower:]')
BASE="totem_${SLUG}_loss"
CAPTURE_ROM="$OUT/$BASE.nds"
CONFIG_DIR="$HOME/.config/desmume"
DSV="$CONFIG_DIR/$BASE.dsv"
POST_SAVE="$OUT/${SLUG}-after-loss.sav"
mkdir -p "$CONFIG_DIR"
...
# The test save has one level-1, 1-HP Pokémon with Memento in every slot.
# Repeated confirmations select the first move and advance defeat/blackout text.
for _ in $(seq 1 65); do
  tap x 0.08
  sleep 0.75
done
sleep 6
shot 02_after_blackout

# Save through Platinum at the healing point, then inspect the active partition.
tap s 0.20
sleep 2
shot 03_menu_open
...
python3 "$ROOT/tools/verify_platinum_save.py" "$POST_SAVE" >"$OUT/post-loss-save-verification.txt"
grep -q 'native_main_load_result=OK' "$OUT/post-loss-save-verification.txt"
python3 "$ROOT/tools/inspect_totem_save_flags.py" "$POST_SAVE" >"$OUT/post-loss-totem-flags.txt"
python3 "$ROOT/tools/assert_totem_save_outcome.py" "$POST_SAVE" "$SLUG" clear >"$OUT/post-loss-flag-assertion.txt"
