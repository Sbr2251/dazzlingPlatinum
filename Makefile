...
#
# The ROM is hardlinked into OUT rather than copied, falling back to cp when OUT
# is on another filesystem.
rom: $(BUILD)/build.ninja data
	$(NINJA) -C $(BUILD) pokeplatinum.us.nds
	mkdir -p $(OUT)
	ln -f $(BUILD)/pokeplatinum.us.nds $(OUT_ROM) 2>/dev/null || cp $(BUILD)/pokeplatinum.us.nds $(OUT_ROM)
