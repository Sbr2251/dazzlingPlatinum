...
// Top-screen logo image, generated by tools/giratina_title/make_top_screen.py.
// Its extended palette is animated: a shine sweeps across the "DAZZLING
// PLATINUM" letters (indices 192 + band * 6 + tone) and the whole screen
// flashes with the prerender's impact frames.
#define LOGO_PALETTE_COLORS       256
#define LOGO_EXT_PLTT_OFFSET      0x4000 // LOGO (sub BG2) uses extended palette slot 2
#define LOGO_PLATINUM_BASE        192
#define LOGO_PLATINUM_BANDS       8
#define LOGO_PLATINUM_TONES       6
#define LOGO_SHINE_PERIOD         240 // Frames between two shine sweeps
#define LOGO_SHINE_FRAMES         40 // How long one sweep takes
#define LOGO_SHINE_HALF_WIDTH     24 // In 1/16 band units
#define LOGO_SHINE_MAX_WEIGHT     14 // Out of 16, toward LOGO_SHINE_COLOR
...
typedef struct TitleScreenPrerender {
    ...
    u16 pendingStep; // step held by the back slot
    volatile u16 displayedStep; // step currently on screen
} TitleScreenPrerender;

typedef struct TitleScreenLogoPalette {
    u16 base[LOGO_PALETTE_COLORS];
    u16 work[LOGO_PALETTE_COLORS];
    u16 shineTimer;
    u8 lastShine;
    u8 lastFlash;
    volatile BOOL uploadPending; // work holds a new palette, upload in VBlank
} TitleScreenLogoPalette;
...
typedef struct TitleScreen {
    ...
    TitleScreenLogoPalette *volatile logoPalette;
} TitleScreen;
...
static void TitleScreen_LoadLogoPalette(TitleScreen *titleScreen, enum HeapID heapID);
static void TitleScreen_UpdateLogoPalette(TitleScreen *titleScreen);
static void TitleScreen_ReleaseLogoPalette(TitleScreen *titleScreen);
#if TITLE_GIRATINA_PRERENDERED
...
static void TitleScreen_VBlank(void *param)
{
    ...
    TitleScreenLogoPalette *logoPalette = appData->titleScreen.logoPalette;

    if (logoPalette != NULL && logoPalette->uploadPending == TRUE) {
        GXS_BeginLoadBGExtPltt();
        GXS_LoadBGExtPltt(logoPalette->work, LOGO_EXT_PLTT_OFFSET, sizeof(logoPalette->work));
        GXS_EndLoadBGExtPltt();
        logoPalette->uploadPending = FALSE;
    }
...
#endif
    TitleScreen_UpdateLogoPalette(titleScreen);
...
#endif
    TitleScreen_ReleaseLogoPalette(titleScreen);
...
static void TitleScreen_Load2DGfx(BgConfig *bgConfig, enum HeapID heapID, TitleScreen *titleScreen)
{
    ...
    TitleScreen_LoadLogoPalette(titleScreen, heapID);
    ...
    // Lavender on dark violet to match the Distortion World top screen
    u16 letterColor = GX_RGB(28, 24, 31);
    u16 shadowColor = GX_RGB(10, 4, 16);
...
{
    int r = BlendColor(ColorR(color), ColorR(target), weight);
    int g = BlendColor(ColorG(color), ColorG(target), weight);
    int b = BlendColor(ColorB(color), ColorB(target), weight);
...
static void TitleScreen_LoadLogoPalette(TitleScreen *titleScreen, enum HeapID heapID)
{
    TitleScreenLogoPalette *logoPalette = Heap_Alloc(heapID, sizeof(TitleScreenLogoPalette));

    GF_ASSERT(logoPalette != NULL);
    memset(logoPalette, 0, sizeof(TitleScreenLogoPalette));

    LoadPaletteFromFile(NARC_INDEX_DEMO__TITLE__TITLEDEMO, logo_NCLR, heapID, sizeof(logoPalette->base), 0, logoPalette->base);

    logoPalette->lastShine = LOGO_SHINE_OFF;
    logoPalette->lastFlash = 0;
    logoPalette->uploadPending = FALSE;

    titleScreen->logoPalette = logoPalette;
}

static void TitleScreen_UpdateLogoPalette(TitleScreen *titleScreen)
{
    TitleScreenLogoPalette *logoPalette = titleScreen->logoPalette;

    if (logoPalette == NULL) {
        return;
    }

    logoPalette->shineTimer++;

    if (logoPalette->shineTimer >= LOGO_SHINE_PERIOD) {
        logoPalette->shineTimer = 0;
    }

    if (logoPalette->uploadPending == TRUE) {
        return;
    }

    // Shine position in 1/16 band units, sweeping from before the first band
    // to past the last one.
    u8 shine = LOGO_SHINE_OFF;

    if (logoPalette->shineTimer < LOGO_SHINE_FRAMES) {
        shine = logoPalette->shineTimer * ((LOGO_PLATINUM_BANDS + 4) * 16) / LOGO_SHINE_FRAMES;
    }
    ...
    if (prerender != NULL && prerender->playing == TRUE) {
        u16 step = prerender->displayedStep;

        if (step >= LOGO_FLASH_FIRST_STEP && step < LOGO_FLASH_FIRST_STEP + NELEMS(sLogoFlashWeights)) {
            flash = sLogoFlashWeights[step - LOGO_FLASH_FIRST_STEP];
        }
    }
#endif

    if (shine == logoPalette->lastShine && flash == logoPalette->lastFlash) {
        return;
    }

    logoPalette->lastShine = shine;
    logoPalette->lastFlash = flash;

    MI_CpuCopy16(logoPalette->base, logoPalette->work, sizeof(logoPalette->work));

    if (shine != LOGO_SHINE_OFF) {
        for (int band = 0; band < LOGO_PLATINUM_BANDS; band++) {
            // Band centres sit at 2.5, 3.5, ... in shine units (2 bands of lead-in)
            int dist = (band + 2) * 16 + 8 - shine;

            if (dist < 0) {
                dist = -dist;
            }

            if (dist >= LOGO_SHINE_HALF_WIDTH) {
                continue;
            }

            int weight = (LOGO_SHINE_HALF_WIDTH - dist) * LOGO_SHINE_MAX_WEIGHT / LOGO_SHINE_HALF_WIDTH;
            u16 *colors = logoPalette->work + LOGO_PLATINUM_BASE + band * LOGO_PLATINUM_TONES;

            for (int tone = 0; tone < LOGO_PLATINUM_TONES; tone++) {
                colors[tone] = TitleScreen_BlendLogoColor(colors[tone], LOGO_SHINE_COLOR, weight);
            }
        }
    }

    if (flash != 0) {
        for (int i = 1; i < LOGO_PALETTE_COLORS; i++) {
            logoPalette->work[i] = TitleScreen_BlendLogoColor(logoPalette->work[i], LOGO_FLASH_COLOR, flash);
        }
    }

    DC_FlushRange(logoPalette->work, sizeof(logoPalette->work));
    logoPalette->uploadPending = TRUE;
}
...
static void TitleScreen_ReleaseLogoPalette(TitleScreen *titleScreen)
{
    ...
    titleScreen->logoPalette = NULL;
    Heap_Free(logoPalette);
}
...
static void TitleScreen_PrerenderVBlank(TitleScreenPrerender *prerender)
{
    ...
    prerender->displayedStep = prerender->pendingStep;
